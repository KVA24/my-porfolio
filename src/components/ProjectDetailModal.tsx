          ...
          >
            <div className="p-4 sm:p-5 border-b border-primary flex justify-between items-center bg-tertiary">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono text-accent font-bold uppercase tracking-widest">Architectural Spec</span>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-sm">
              ...
              <div className="space-y-1.5 pt-2">
                <h4 className="text-strong font-extrabold text-xs uppercase tracking-wider text-accent">{t('projects.challenge')}</h4>
                <p className="text-primary leading-relaxed text-xs">{project.details.challenge}</p>
              </div>

              {/* Technical Solution */}
              <div className="space-y-1.5">
                <h4 className="text-strong font-extrabold text-xs uppercase tracking-wider text-emerald-400">{t('projects.solution')}</h4>
                <p className="text-primary leading-relaxed text-xs">{project.details.solution}</p>
              </div>

              {/* Core metrics visual grid */}
              <div className="space-y-2">
                <h4 className="text-strong font-extrabold text-xs uppercase tracking-wider text-[#58a6ff]">{t('projects.metrics')}</h4>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {project.details.metrics.map((metric, index) => (
                    <div key={index} className="p-3 rounded-lg bg-tertiary border border-primary text-xs text-strong font-semibold flex items-start gap-2">
              ...
              <div className="space-y-2">
                <h4 className="text-strong font-extrabold text-xs uppercase tracking-wider text-purple-400">{t('projects.architecture')}</h4>
                <div className="bg-tertiary p-4 rounded-lg border border-primary space-y-3 font-mono text-xs text-secondary">
